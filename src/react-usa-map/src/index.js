...
class USAMap extends React.Component {
  ...
  buildPaths = () => {
    ...
    for (let stateKey in data) {
      const path = <USAState showStateTooltip={this.props.showStateTooltip} key={stateKey} stateName={data[stateKey].name} dimensions={data[stateKey]["dimensions"]} state={stateKey} fill={this.fillStateColor(stateKey)} onClickState={this.stateClickHandler(stateKey)} />
      paths.push(path);
    };
    return paths;
  };
  ...
  render() {
    return (
      <svg className="us-state-map" xmlns="http://www.w3.org/2000/svg" width={this.props.width} height={this.props.height} viewBox="0 0 959 593">
        {this.props.showStateTooltip && <title>{this.props.title}</title>}
...
USAMap.propTypes = {
  ...
  showStateTooltip: PropTypes.bool
};
